...
import { applySoundState, createSoundButton } from "../utils/SoundManager";
...
export class MainMenu extends Scene {
  ...
  create() {
    ...
    let bgm = this.sound.get("soundgame");

    if (!bgm) {
      bgm = this.sound.add("soundgame", {
        loop: true,
        volume: 0.5,
      });
    }
  ...
  private showMatchExpiredModal(data: any) {
    ...
    const message = this.add
      .text(0, 10, data?.message || "Kedua pemain terputus terlalu lama.\nMatch ini dianggap tidak valid.", {
        fontFamily: "Poppins",
        fontSize: "14px",
        color: "#FFFFFF",
        align: "center",
    ...
    modal.add([card, icon, title, message, subMessage, buttonBg, buttonText, hitArea]);
